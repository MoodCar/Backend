...
exports.userGetAll = async function () {
  try {
    ...
    try {
      ...
      let [row] = await connection.query(getUserQuery);
      connection.release();
      return row;
    } catch (err) {
...
exports.userFetchInfo = async function (providerId) {
  try {
    ...
    console.log(`##### Connection_pool_GET #####`);
    try {
      let fetchUserInfoQuery = "select * from user where providerId = ?";
      let params = providerId;
      let [row] = await connection.query(fetchUserInfoQuery, params);
      connection.release();
      return row;
    } catch (err) {
      console.error(`##### Query error ##### `);
...
exports.userUpdate = async function (providerId, location, preference) {
  try {
    ...
    try {
      let updateUserQuery =
        "update user set location = ?, preference = ? where providerId = ?";
      let params = [location, preference, providerId];
      let [row] = await connection.query(updateUserQuery, params);
      if (row.changedRows == 1) {
        params = providerId;
        let getUserQuery =
          "select location,preference from user where providerId = ?";
